...
INPUT_PATH = '/home/yuan/CAMIOT/frameObj0.jpg'

# pi version
# CHECKPOINT_FILE = '/home/pi/Camiot/epoch_300.pth'
# INPUT_PATH = '/home/pi/Camiot/desk_chair/'
...
#         continue
#     img = cv2.imread(osp.join(INPUT_PATH, item), cv2.IMREAD_COLOR).astype(np.float32)
#     print('reading time: ', time.time() - time_start_a)
#
#     time_start_b = time.time()
#     result_prob = appliance_recognition(img, model)
#     print(result_prob)
#     print('processing time: ', time.time() - time_start_b)

# local version
time_start_a = time.time()
if '.jpg' not in INPUT_PATH and '.png' not in INPUT_PATH:
    print('not a valid image')
else:
    img = cv2.imread(INPUT_PATH, cv2.IMREAD_COLOR).astype(np.float32)
    print('reading time: ', time.time() - time_start_a)

    time_start_b = time.time()
    result_prob, result_items = appliance_recognition(img, model)
    print(result_prob, result_items)
    print('processing time: ', time.time() - time_start_b)
