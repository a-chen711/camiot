...
while True:
	...
	time.sleep(0.5)
	result_prob=appliance_recognition(imgName)
	print('Finished appliance recognition')
	appliance=result_items[obj_num] #most probable result is the first one
	#call(["python3", "speak.py", appliace])
	print('Appliance recognized was ' + appliance)
	#engine.say(appliace)
	#engine.runAndWait()
	print(result_items)
	counter2 += 1
	...
	# conn0.sendall(pack)
	#send first index of result_items to the pi since it is the highest prob and therefore the chosen appliance
	#
